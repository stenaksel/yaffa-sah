<!-- Generated by BASH-STUFF — do not edit directly.
     Edit alias_*.yaml or files in config/func/bash/ and re-run YAFFA generator. -->

# BASH-STUFF — Available Aliases & Functions

Ready-to-use bash aliases and functions. These files are generated — do not edit them directly.
To add or modify aliases, see the [generator README](https://github.com/stenaksel/YAFFA#readme).

## Setup

Your `~/.bashrc` is a script Bash runs automatically every time it starts —
...
```bash
readonly YAFFA_GEN="$HOME/.yaffa/gen/bash-stuff"
...
### Default - some common aliases (`alias_default.sh`)
...
| `rm_hist` | `history -c && history -w` | Clear command history (current session and saved history file) — irreversible |
| `hist-c` | `history -c && history -w` | Clear command history (current session and saved history file) — irreversible |
| `ll` | `ls -la --color=auto` | Long listing including hidden files |
| `shell_` | `bash --version` | Display shell version information |
| `term_` | `bash --version` | Display shell version information |

### Git aliases (`alias_git.sh`)
...
### Gradle aliases (`alias_gradle.sh`)
...
| `pva` | `.venv/Scripts: source .venv/Scripts/activate · .ve…` | Activate the .venv virtual environment in the current shell |
...
### YAFFA - regenerate your aliases (`alias_yaffa.sh`)
...
| `_YaffaCall <name> <cmd> <desc>` | `name` — Alias name, shown in the diagnostic line, `cmd` — Shell command to eval, `desc` — Optional description, printed in yellow above the diagnostic line (each line of a multi-line desc gets its own line) | Print an optional description (yellow), followed by a "<name> => <cmd>" diagnostic (cmd in cyan), then eval cmd, forwarding any extra args |
